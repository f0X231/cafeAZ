<!-- MENU -->
<nav class="pushy pushy-left">
    <div class="btn-close-menu-box"><img src="images/icon-close-white.png" class="btn-close-menu" /></div>
    <ul class="menu-main">
        <li class="pushy-link"><a href="index.php">หน้าแรก</a></li>
        <li class="pushy-submenu">
            <a href="javascript:void(0);">สินค้า</a>
            <ul>
                <li class="pushy-link pushy-link-submenu"><a href="products.php" class="bg_white">&#8226;&nbsp;เครื่องดื่ม</a></li>
                <li class="pushy-link pushy-link-submenu"><a href="products.php" class="bg_white">&#8226;&nbsp;สินค้าอื่นๆ</a></li>
                <li class="pushy-link pushy-link-submenu"><a href="products.php" class="bg_white">&#8226;&nbsp;เครื่องดื่มพิเศษ</a></li>
            </ul>
        </li>
        <li class="pushy-link"><a href="promotion.php">โปรโมชั่น</a></li>
        <li class="pushy-link"><a href="news.php">ข่าวและอัพเดต</a></li>
        <li class="pushy-link"><a href="ourbranch.php">ร้านและสาขา</a></li>
        <li class="pushy-link"><a href="franchise.php">แฟรนไชส์</a></li>
        <li class="pushy-submenu">
            <a href="javascript:void(0);">ศูนย์ธุรกิจ คาเฟ่ อเมซอน</a>
            <ul>
                <li class="pushy-link pushy-link-submenu"><a href="aica.php" class="bg_white">&#8226;&nbsp;ศูนย์ธุรกิจ คาเฟ่ อเมซอน</a></li>
                <li class="pushy-link pushy-link-submenu"><a href="factory.php" class="bg_white">&#8226;&nbsp;โรงคั่วกาแฟ คาเฟ่ อเมซอน</a></li>
                <li class="pushy-link pushy-link-submenu"><a href="training.php" class="bg_white">&#8226;&nbsp;ศูนย์ฝึกอบรม</a></li>
            </ul>
        </li>
        <li class="pushy-link"><a href="tips.php">เกร็ดความรู้ เรื่องกาแฟ</a></li>
        <li class="pushy-link"><a href="contact.php">ติดต่อเรา</a></li>
    </ul>
    <div id="menu-footer-groups">
        <div class="menu-footer"><a href="about.php">&#8226;&nbsp;เกี่ยวกับ คาเฟ่ อเมซอน</a></div>
        <div class="menu-footer"><a href="faq.php">&#8226;&nbsp;คำถามที่พบบ่อย</a></div>
        <div class="menu-footer"><a href="csr.php">&#8226;&nbsp;ความรับผิดชอบต่อสังคม ชุมชน สิ่งแวดล้อม</a></div>
    </div>
</nav>

<!-- Site Overlay -->
<div class="site-overlay"></div>


<!-- Menu Button -->
<div class="menu-btn noactive"><i class="fa fa-bars fa-2x" aria-hidden="true"></i></div>

<!-- logo -->
<div class="logo-of-website hide-for-small-only hide-for-medium-only noactive">
    <a href="index.php"><img src="images/logo.png" alt="logo amazon cafe" /></a>
</div>
<div class="logo-of-website-for-ipad show-for-medium-only noactive">
    <a href="index.php"><img src="images/logo-for-ipad.png" alt="logo amazon cafe" /></a>
</div>
<div class="logo-of-website-for-mobile show-for-small-only noactive">
    <a href="index.php"><img src="images/logo-for-mobile.png" alt="logo amazon cafe" /></a>
</div>

<!-- Switch Language -->
<div class="menu-switch-language noactive">
    <div class="switch-language-facebook hide-for-small-only">
        <a target="_blank" href="http://www.facebook.com/cafeamazonbyptt/">
            <div class="menu_share_facebook"></div>
        </a>
    </div>
    <div class="show-switch-language">Thai</div>
</div>
